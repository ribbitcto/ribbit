...
/**
 * Glitch scroll effect that creates an RGB reality distortion
 * while smoothly scrolling to the buy section
 */
function glitchScrollToBuy() {
    ...
    // Create glitch overlay with inline styles as fallback
    ...
    // Add glitch effects to body
    document.body.classList.add('glitch-active');
    // Fallback inline animation for body
    document.body.style.animation = 'glitchPulse 0.5s ease-in-out';
    console.log('🐸 Body glitch effect activated');
    
    // Create RGB shift effect on text elements
    const textElements = document.querySelectorAll('h1, h2, h3, h4, p, span, .logo');
    console.log('🐸 Found', textElements.length, 'text elements for RGB shift');
    
    textElements.forEach((el, index) => {
        setTimeout(() => {
            el.classList.add('rgb-shift');
            // Inline RGB shift animation as fallback
            el.style.animation = 'rgbShift 0.3s ease-out';
            el.style.textShadow = '-2px 0 0 red, 2px 0 0 green, 0 2px 0 blue';
            setTimeout(() => {
                el.classList.remove('rgb-shift');
                el.style.animation = '';
                el.style.textShadow = '';
            }, 300);
        }, index * 20);
    ...
    // Scroll to buy section with custom easing
    setTimeout(() => {
        ...
        console.log('🐸 Buy section found, starting scroll...');
        
        // Calculate distance for dramatic effect
        const targetPosition = buySection.offsetTop - 80; // Account for navbar
        const startPosition = window.pageYOffset;
        const distance = targetPosition - startPosition;
        const duration = 1200; // Slightly longer for dramatic effect
        let start = null;
        ...
        // Custom glitch easing function
        function glitchEase(t) {
            // Add some random glitch stutters
            const glitchFactor = Math.random() < 0.1 ? Math.random() * 0.1 : 0;
            return t < 0.5 
                ? 4 * t * t * t + glitchFactor
                : 1 - Math.pow(-2 * t + 2, 3) / 2 + glitchFactor;
        }
        
        function animateScroll(timestamp) {
            if (!start) start = timestamp;
            const progress = Math.min((timestamp - start) / duration, 1);
            const easeProgress = glitchEase(progress);
            ...
            if (progress < 1) {
                ...
                console.log('🐸 Scroll animation complete, cleaning up...');
